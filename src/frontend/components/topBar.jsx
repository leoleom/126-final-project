...
function TopBar({ user, searchQuery, setSearchQuery }) {
  ...
  const handleProfileClick = () => {
    console.log(`TopBar: Profile link clicked for user @${user?.username}`);
  };

  const handleSearchChange = (e) => {
    // Logging the search input interaction
    console.log("TopBar: Search input changed");
    ...
    <header className="flex h-28 items-center justify-between border-b border-[#e5e7eb] px-9">
      <div className="flex h-12 w-[480px] items-center rounded-lg border border-[#e5e7eb] bg-[#f9fafb] px-4 text-sm text-[#6b7280]">
        ...
        <input
          type="text"
          value={searchQuery}
          onChange={handleSearchChange} // Updated to include logging
      ...
      <Link 
        to="/profile" 
        onClick={handleProfileClick} // Added listener
